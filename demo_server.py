...
async def main():
    global pose_mat, trans, dt, reset_offset, offset_height, superfast, j3d, j2d, num_ppl, bbox, frame, tracking_res, images_acc, mdm_talker
    offset = np.zeros((5, 1))
...
async def pose_getter(request):
    ...
    global pose_mat, trans, dt, j3d, mode, ticker
    curr_paths = {}
...
def generate_text(prompt):
    global  mdm_motions, offset_height, mdm_talker
    mdm_motions, abs_path = mdm_talker.generate_text(prompt)
    mat = sRot.from_euler('xyz', np.array([-np.pi / 2, 0, 0]), degrees=False).as_matrix()
    mdm_motions = np.matmul(mdm_motions, mat.dot(mat))
    
    offset = - offset_height - mdm_motions[0, 0, 1]
    mdm_motions[..., 1] += offset
    mdm_motions = np.concatenate([np.repeat(mdm_motions[0:1], 30, axis = 0), mdm_motions])
    path = osp.join(abs_path, "sample00_rep00.mp4")


async def talk_server(request):
    global ws_talkers, mode, mdm_motions, to_metrabs, offset_height, reset_offset
    post = await request.text()
    json_post = json.loads(post)
    try:
        if json_post['action'] == "set_mode":
            print(f"Setting mode to {json_post['query']['mode']}")
            mode = json_post['query']['mode']
            return web.Response(text=f'Setting mode to {mode}')
...
if __name__ == "__main__":
    ...
    mode, mdm_motions, ticker, to_metrabs = "language", np.zeros([120, 24, 3]), 0, sRot.from_euler('xyz', np.array([-np.pi / 2, 0, 0]), degrees=False).as_matrix()
