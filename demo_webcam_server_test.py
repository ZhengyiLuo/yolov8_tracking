...
def frames_from_webcam():
    ...
    while (cap.isOpened()):
        ...
        yolo_output = det_model([frame], size=320)
        yolo_out_xyxy = torch.stack(yolo_output.xyxy)
        
        if yolo_out_xyxy.shape[1] > 0:
            yolo_out_xyxy = yolo_out_xyxy.cpu().numpy()
            select = yolo_out_xyxy[0, :, 4] > detec_threshold
            yolo_out_xyxy = yolo_out_xyxy[:, select]
            num_ppl = yolo_out_xyxy.shape[1]
            
            bbox = np.stack([yolo_out_xyxy[0, :, 0], yolo_out_xyxy[0, :, 1], (yolo_out_xyxy[0, :, 2] - yolo_out_xyxy[0, :, 0]), (yolo_out_xyxy[0, :, 3] - yolo_out_xyxy[0, :, 1])], axis = 1)
            
            for i in range(num_ppl):
                x1, y1, x2, y2 = yolo_out_xyxy[0, i, :4]
